...
void PostStorageHandler::ReadPost(
  ...
  input_ss << post_id_str;
  output_ss << _return.req_id << ',' << _return.timestamp << ',' << _return.post_id << ',' << _return.creator.user_id 
  << ',' << _return.creator.username << ',' << _return.post_type << ',' << _return.text;
  input_str = input_ss.str();
  output_str = output_ss.str();
  input_hash = str_hash(input_str);
  output_hash = str_hash(output_str);
  std::cout << "[FunctionLog]: ReadPostStorageHandler," << input_hash << "," << output_hash << "," << duration.count() << std::endl;
...
void PostStorageHandler::ReadPosts(
  ...
  // 计算时间差，转换为毫秒
  std::chrono::duration<double> duration = end_time - start_time;
  input_ss << vecToString(post_ids);
  output_ss << vecToString(_return);
  input_str = input_ss.str();
  output_str = output_ss.str();
  input_hash = str_hash(input_str);
  output_hash = str_hash(output_str);
  std::cout << "[FunctionLog]: ReadPostsStorageHandler," << input_hash << "," << output_hash << "," << duration.count() << std::endl;
  // Clear the str concat
  output_ss.str("");
  input_ss.str("");
}
