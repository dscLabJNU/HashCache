...
void TextHandler::ComposeText(
  ...
  auto shortened_urls_future = std::async(std::launch::async, [&]() {
    ...
    // 计算时间差，转换为毫秒
    std::chrono::duration<double> duration = end_time - start_time2;
    input_ss << vecToString(urls);
    output_ss  << vecToString(_return_urls);
    input_str = input_ss.str();
    output_str = output_ss.str();
    input_hash = str_hash(input_str);
    output_hash = str_hash(output_str);
    std::cout << "[FunctionLog]: ComposeUrls," << input_hash << "," << output_hash << "," << duration.count() << std::endl;
  ...
  auto user_mention_future = std::async(std::launch::async, [&]() {
    ...
    auto end_time = std::chrono::high_resolution_clock::now();
    // 计算时间差，转换为毫秒
    std::chrono::duration<double> duration = end_time - start_time3;
    input_ss << vecToString(mention_usernames);
    output_ss  << vecToString(_return_user_mentions);
    input_str = input_ss.str();
    output_str = output_ss.str();
    input_hash = str_hash(input_str);
    output_hash = str_hash(output_str);
    std::cout << "[FunctionLog]: ComposeUserMentions," << input_hash << "," << output_hash << "," << duration.count() << std::endl;
  ...
  auto end_time = std::chrono::high_resolution_clock::now();
  // 计算时间差，转换为毫秒
  std::chrono::duration<double> duration = end_time - start_time1;
  input_ss << text;
  output_ss << _return.text << ',' << vecToString(_return.urls) << ',' << vecToString(_return.user_mentions);
  input_str = input_ss.str();
  output_str = output_ss.str();
  input_hash = str_hash(input_str);
  output_hash = str_hash(output_str);
  std::cout << "[FunctionLog]: ComposeText," << input_hash << "," << output_hash << "," << duration.count() << std::endl;
  // Clear the str concat
  output_ss.str("");
  input_ss.str("");
  span->Finish();
}
