...
print("func_name,hash_input,hash_output,duration", file=log_csv, flush=True)

def record(func_name, hash_input, hash_output, duration):
  print(f"{func_name},{hash_input},{hash_output},{duration}", file=log_csv, flush=True)

async def upload_cast_info(session, addr, cast):
  func_name = "upload_cast_info"  
  hash_input=hash(str(cast))
  start = time.time()
  async with session.post(addr + "/wrk2-api/cast-info/write", json=cast) as resp:
    text = await resp.text()
    duration = time.time() - start
    hash_output = hash(text)
    record(func_name, hash_input, hash_output, duration)   
    print(f"Output of {func_name} is: {text}") 
    return text
    # return await resp.text()

async def upload_plot(session, addr, plot):
  func_name = "upload_plot"
  start = time.time()
  hash_input=hash(str(plot))
...
async def upload_movie_info(session, addr, movie):
  func_name = "upload_movie_info"
  start = time.time()
  hash_input=hash(str(movie))
...
async def register_movie(session, addr, movie):
  ...
  func_name = "register_movie"
  start = time.time()
  hash_input=hash(str(params))
