...
trait DocumentFactory[W <: DocumentRevisionProvider] extends MultipleReadersSingleWriterCache[W, DocInfo] {
  ...
  def putAndAttach[Wsuper >: W](db: ArtifactStore[Wsuper],
    ...
    logger.info(this, s"doc in DocumentFactory are: $doc, key is $key, actionRelationShipPostedFuture keys are: ${HashCache.actionRelationShipPostedFuture.keys}")
    for (actionPramsCode <- HashCache.actionRelationShipPostedFuture.keys){
      logger.info(this, s"searching key [${actionPramsCode.toString()}, ${doc}]")
      if (doc.toString.contains(actionPramsCode.toString())){
        logger.info(this, s"key hit!: ${key.toString()}, clearing cache entities")
        HashCache.numExecutionsAction(actionPramsCode).clear()
        HashCache.actionRelationShipPostedFuture(actionPramsCode).clear()
      }
    }
    val p = Promise[W]
