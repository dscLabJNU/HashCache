...
protected[core] case class ActivationResponse private (statusCode: Int,
                                                       ...
                                                       hasIOAccess: Option[Boolean] = None) {
...
protected[core] object ActivationResponse extends DefaultJsonProtocol {
  ...
  protected[core] def success(result: Option[JsValue] = None, size: Option[Int] = None, hasIOAccess: Option[Boolean] = None) =
    ActivationResponse(Success, result, size, hasIOAccess)
              ...
              case scala.util.Success(result @ JsObject(fields)) =>
                ...
                if (res.okStatus) {
                  errorOpt map { error =>
                    ...
                    // The happy path.
                    // For IO access, extract io access flag
                    val hasIOAccess = {
                      if (result.getFields("is_open").size > 0)
                        result.getFields("is_open").head match { case JsBoolean(v) => Some(v) }
                      else
                        None
                    }

                    val payloadResult = {
                      if (result.getFields("run").size > 0)
                        result.getFields("run").headOption
                      else
                        Some(result)
                    }

                    success(payloadResult, sizeOpt, hasIOAccess)
                  }
                } else {
  ...
  protected[core] implicit val serdes = jsonFormat4(ActivationResponse.apply)
}
