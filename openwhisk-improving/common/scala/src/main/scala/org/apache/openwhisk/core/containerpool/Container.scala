...
import org.apache.openwhisk.core.containerpool.Plugin.HashCache
...
trait Container {
  ...
  def run(parameters: JsObject,
    ...
    // Hashing parameters
    val actionPramsCode = HashCache.getHashCode(parameters)
    // logging.info(this, s"params are: $parameters, actionPramsCode is $actionPramsCode")
    val cacheStart = Instant.now()
    if (HashCache.matchCacheConditions(actionName, actionPramsCode)) {
      val cachedResult = HashCache.actionRelationship(actionName)(actionPramsCode).head
      val cacheEnd = Instant.now()
      val interval = Interval(start = cacheStart, end = cacheEnd)
      logging.info(this, s"Cached output is: $cachedResult, interval is : $interval")
      Future {
        (interval, cachedResult)
      }
    }
    else { // data cached miss
      val start =
        transid.started(
          this,
      ...
      callContainer("/run", body, timeout, maxConcurrent, retry = false, reschedule)
        ...
        .map { result =>
          ...
          if (HashCache.enableCache && response.result.isDefined && !response.hasIOAccess.getOrElse(true))
            HashCache.cacheAction(actionName, actionPramsCode, response)
