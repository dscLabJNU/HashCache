...
protected[actions] trait PrimitiveActions {
  ...
  private def invokeSimpleAction(
    ...
    val actionName = "/" + action.namespace + "/" + action.name
    val actionPramCode = HashCache.getHashCode(args.get)

    val postedFuture = if (HashCache.matchCacheConditionsPostedFuture(actionName, actionPramCode)) {
      // Do not need to publish the message for an invoker
      HashCache.actionRelationShipPostedFuture(actionName)(actionPramCode).last
    } else {
      val postedFuture = loadBalancer.publish(action, message)
      HashCache.cacheWhiskPostedFuture(actionName, actionPramCode, postedFuture)
      postedFuture
    }
