      ...
      container
        .andThen {
          case Success(container) =>
            logging.info(this, s"Container Startup Time: ${Interval(startupInit, Instant.now()).duration}")
    ...
    val initialize = stateData match {
      ...
      case _ =>
        ...
        val result = container
          ...
          .map(Some(_))
        logging.info(this, s"Container Initialize Time: ${Interval(initStart, Instant.now()).duration}")
        result
    }
        ...
        container
          ...
          .map {
            case (runInterval, response) =>
              ...
              logging.info(this, s"Container Execute Time: ${Interval(runStart, Instant.now()).duration}")
  ...
  private def logTruncatedError(act: WhiskActivation) = {
    ...
    logging.warn(
      ...
      s"Activation ${
        ...
        stateData.getContainer
      } (with $activeCount still active) returned a $errorTypeMessage: $truncatedResult")
  }
}
