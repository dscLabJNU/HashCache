...
public class PriceConfigRepositoryImpl implements PriceConfigRepository {
    ...
    @Override
    public boolean init(ArrayList<PriceConfig> priceConfigs) {
        try {
            for (PriceConfig priceConfig : priceConfigs) {
                // System.out.println("priceConfig: "+JsonUtils.object2Map(priceConfig));
                Document doc = new Document(JsonUtils.object2Map(priceConfig));
