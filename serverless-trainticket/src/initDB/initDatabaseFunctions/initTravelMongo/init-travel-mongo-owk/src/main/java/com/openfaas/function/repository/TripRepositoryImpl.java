...
public class TripRepositoryImpl implements TripRepository {
    ...
    @Override
    public boolean init(ArrayList<TravelInfo> travelInfos){
        try {
            for (TravelInfo travelInfo : travelInfos) {
                travelInfo.setStartingTime(new Date("Mon May 04 09:00:00 GMT+0800 2013"));
                travelInfo.setEndTime(new Date("Mon May 04 15:51:52 GMT+0800 2013"));
                Trip trip = new Trip(new TripId(travelInfo.getTripId()), travelInfo.getTrainTypeId(), travelInfo.getStartingStationId(),
                travelInfo.getStationsId(), travelInfo.getTerminalStationId(), travelInfo.getStartingTime(), travelInfo.getEndTime());
                trip.setRouteId(travelInfo.getRouteId());
                System.out.println("TripInfo: "+ JsonUtils.object2Map(trip));
                
                Document doc = new Document(JsonUtils.object2Map(trip));
