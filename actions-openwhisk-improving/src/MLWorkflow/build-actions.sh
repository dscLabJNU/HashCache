...
function clear_api(){
  ...
  # 在输出结果中查找是否有 API Name 为 /MLWorkflow 的 API
  if echo "$api_list" | grep -q "/MLWorkflow"; then
    echo "API /MLWorkflow found, deleting..."
    
    # 执行 wsk -i api delete /MLWorkflow 命令来删除 API
    wsk -i api delete /MLWorkflow
    
    # 检查 API 是否被成功删除
    if [ $? -eq 0 ]; then
      echo "API /MLWorkflow deleted successfully"
    else
      echo "Failed to delete API /MLWorkflow"
    fi
  else
    echo "API /MLWorkflow not found"
  fi
}
...
function do_build(){
    ...
    # 检查 API 数量是否为 4
    local count=$(wsk -i api list | grep /MLWorkflow/ | wc -l)
    if [ $count -ne 4 ]; then
      echo "API count is $count, retrying..."
      # 如果 API 数量不为 4，继续执行 do_build 函数
      return 1
    fi
